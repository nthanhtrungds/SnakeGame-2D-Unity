...
public class Snake : MonoBehaviour {
    ...
    {
        int number = snakeBodyPartList.Count;

        if (number >= 5) gridMoveTimerMax = .25f;
        if (number >= 10) gridMoveTimerMax = .20f;
        if (number >= 15) gridMoveTimerMax = .15f;
        if (number >= 20) gridMoveTimerMax = .10f;
        if (number >= 25) gridMoveTimerMax = .05f;
    }
